...
/*   Updated: 2021/06/09 19:50:25 by moerradi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	keep_rotating(t_stack **stack, int pivot, int direction, bool verbose)
{
	int	c;
	int	size;

	size = 	size = stack_size(*stack);
	c = 0;
...
int	rotate_optimizer(t_stack *stack, int pivot)
{
	t_stack	*r;
	t_stack	*rr;
	int		rc;
...
void	go(t_env *e)
{
	...
	while (stack_size(e->a) > 3)
	{
		...
		{
			keep_rotating(&e->a, pivot, rotate_optimizer(e->a, pivot), true);
			push(&e->a, &e->b, chunk);
		}
		chunk++;
	}
	...
	while (stack_size(e->b) > 3)
	{
		...
		{
			if ()
			rn += keep_rotating(&e->a, pivot, 1, true, chunk);
			push(&e->b, &e->a, chunk);
		}
		chunk--;
	}
